<template>
    <div class="challo__card challo__card-p6 rounded-b-3xl">
        <h5 class="page-title">Neuer Vertriebspartner hinzufügen</h5>
        <div class="form-wrapper">
            <form class="classic-form" @submit.prevent="create">
                    ...
                    <div class="input-wrapper w-1/3 flex">
                        <div class="flex gap-4">
                            <label for="profile-picture"
                                ...
                                <img v-if="form.profile_picture" :src="profile_picture" alt="profile_picture" />
                ...
                <div class="!mb-10 form-field">
                    <div class="input-wrapper w-1/3">
                        <TextInput maxlength="41" placeholder="Firmenname eingeben" label="Firmenname*"
                            ...
                            :error="v$.company_name.$errors[0]?.$message || form.errors.company_name"
                    ...
                    <div class="input-wrapper w-1/3">
                        ...
                        <label for="receipt_template"
                            ...
                            <Upload fill="#135F84" />
                            <p class="value !m-[0px]" v-if="!form.receipt_template">
                                Belevorlage hochladen .jpg oder .png Format
                            </p>
                            <div class="flex w-full justify-between" v-if="form.receipt_template_name">
                                <p class="value !text-16 !m-0 text-tints-5 whitespace-nowrap overflow-hidden text-ellipsis max-w-[70%]">
                                    {{ form.receipt_template_name }}
                                </p>
                ...
                <div class="!mb-10 form-field">
                    ...
                    <div class="input-wrapper w-1/3">
                        ...
                        <SelectOption name="branch_categories" v-model="form.branch_category_id"
                            ...
                            ]" :error="form.errors.branch_category_id" @change="form.clearErrors('branch_category_id')" />
                    </div>
                ...
                <h2 class="w-full text-primary-3 mb-10 text-18x font-semibold">Adresse</h2>
                ...
                <div class="!mb-10 form-field">
                    ...
                    <div class="input-wrapper w-1/3">
                        <TextInput placeholder="Hausnummer eingeben" label="Hausnummer*" v-model="form.house_number"
                            :error="v$.house_number.$errors[0]?.$message || form.errors.house_number" @clearError="
                              () => {
                    ...
                    <div class="input-wrapper w-1/3">
                        <TextInput placeholder="0000" label="Postleitzahl*" v-model="form.zip_code" :error="
                          form.errors.zip_code ||
                          (form.country == 'ch'
                            ? form.zip_code.length > 4
                            : form.zip_code.length > 10)
                            ? 'error'
                            : undefined
                        " @clearError="
                () => {
              ...
              " :maxLength="form.country == 'ch' ? 5 : 11" :show_error="false" />
                        ...
                        <div class="error text-error font-ropa" v-if="
                          form.country == 'ch'
                            ? form.zip_code.length > 4
                            : form.zip_code.length > 10
                        ">
                ...
                <h2 class="w-full text-primary-3 mb-10 text-18x font-semibold">Kontaktperson</h2>

                <div class="!mb-10 form-field">
                    <div class="input-wrapper w-1/3">
                        <TextInput placeholder="Vorname eingeben" label="Vorname*"
              ...
              " maxLength="31" />
                    ...
                    <div class="input-wrapper w-1/3">
                        <TextInput placeholder="Nachname eingeben" label="Nachname*"
              ...
              " :show_error="v$.contact_person_email.$errors[0]?.$message ? true : false"
                ...
                <h2 class="w-full text-primary-3 mb-10 text-18x font-semibold">Verträge</h2>
                ...
                <div class="form-field-wrapper" v-if="form.contract">
                    <div class="!mb-10 form-field">
                        <div class="input-wrapper w-1/3">
                            <label class="detail__label block mb-2">Vertrags-ID</label>
                            <p class="detail__value">{{ form.contract.prefix_id }}</p>
                        </div>
                        <div class="input-wrapper w-1/3">
                            <label class="detail__label block mb-2">Vertragsnummer/-bezeichnung</label>
                            <p class="detail__value">{{ form.contract.name }}</p>
                        </div>
                        <div class="input-wrapper w-1/3">
                            <label class="detail__label block mb-2">Vertragslaufzeit</label>
                    ...
                    <div class="!mb-10 form-field">
                        <div class="input-wrapper w-1/3">
                            <label class="detail__label block mb-2">Paket</label>
                            <a target="_blank" class="detail__value underline underline-offset-4"
                                :href="route('package.show', { package: form.contract.package.id })">{{
                                form.contract.package.package_name }}</a>
                        </div>
                        <div class="input-wrapper w-1/3">
                            <label class="detail__label block mb-2">Marketinggebühr</label>
                            <a target="_blank" class="detail__value underline underline-offset-4" :href="
                ...
                <div class="!mb-10 form-field" v-if="form.contract">
                    <button @click="
                      form.post(route('sales-partner.set-session.redirect', { to_edit: true }))
                    " type="button" class="challo__btn bg-primary-2 w-[417px] flex gap-[14px] justify-center items-center">
                        <Pen /> <span>Vertrag bearbeiten</span>
                    </button>
                </div>

                <div class="!mb-10 form-field" v-if="!form.contract">
                    <button @click="form.post(route('sales-partner.set-session.redirect'))" type="button"
                        class="challo__btn bg-primary-2 w-[417px] flex gap-[14px] justify-center items-center">
                        <Plus /> <span>Neuer Vertrag hinzufügen</span>
                    </button>
                </div>

                <div class="!mb-6 form-field">
                    <div class="input-wrapper flex items-center">
                        <SwitchInput :statusText="false" v-model="status" />
                        <p class="switch__input--label">Vertriebspartner aktivieren</p>
                    </div>
                </div>
                ...
                <div class="!mb-[18px] form-field row">
                    ...
                    <div class="input-wrapper w-1/3">
                        <Cancel :target="route('sales-partner.index')"
                            message="Registrierung des Vertriebspartners abbrechen?"
                            text="Wenn Sie diesen Vorgang abbrechen, werden alle Daten verworfen. Sind Sie sicher, dass Sie die Registrierung dieses Vertriebspartners wirklich abbrechen wollen?" />
                    </div>
                </div>
            </form>
        </div>
...
import { computed, inject, watch, onMounted } from "@vue/runtime-core";
...
const form = useForm({
    ...
    receipt_template: sales_partner?.receipt_template ?? "",
    ...
    contract: sales_partner?.contract ?? null,
    profile_picture: sales_partner?.profile_picture ?? "",
    ...
    country: sales_partner?.country_iso ?? "ch",
    no_information: sales_partner?.no_information ?? false,
...
const status = computed({
    get: () => {
        return form.status == "active" ? true : false;
    },
    set: (value) => {
        value ? (form.status = "active") : "new";
    },
});
...
const create = async () => {
    form.post(route("sales-partner.store"), {
        forcedData: true,
