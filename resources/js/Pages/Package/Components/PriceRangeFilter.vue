<template>
    ...
    <div class="filter-price-range  shadow-md rounded-b-md flex-col min-w-[320px]" v-if="openOther" v-click-away="close">
        <div class="flex items-center justify-between w-full">
            <input
                id="price_start"
                class="challo__input"
                v-model="priceForm.inputPrice.start"
            />
            <span class="">
            ...
            <input
                id="price_end"
                class="challo__input"
                v-model="priceForm.inputPrice.end"
            />

            <button class="clear-btn challo__btn" @click.prevent="clearPrice">Löschen</button>
        </div>
...
export default {
    ...
    setup(props, ctx) {
        ...
        //console.log( priceForm.price );

        const start_price_key = computed(()=>props.keyPrefix ? `${props.keyPrefix}_start_price` : 'start_price')
        const end_price_key = computed(()=>props.keyPrefix ? `${props.keyPrefix}_end_price` : 'end_price')
        ...
        const setPriceFromUrl = () => {
            ...
            if (start && end) {
                priceForm.price.start = priceForm.inputPrice.start = start;
                priceForm.price.end = priceForm.inputPrice.end = end;
            }
        };
        ...
        watch(
            ...
            _debounce(function () {
                ...
                priceForm.inputPrice.start = ( ( priceForm.inputPrice.start === null || priceForm.inputPrice.start === '' ) && priceForm.kePress ) ? priceForm.inputPrice.start : priceForm.price.start;
                priceForm.inputPrice.end = ( ( priceForm.inputPrice.end === null || priceForm.inputPrice.end === '' ) && priceForm.kePress ) ? priceForm.inputPrice.end : priceForm.price.end;
                priceForm.is_default = false;
                priceForm.kePress = false;
        ...
        watch(
            () => priceForm.inputPrice,
            _debounce(function () {
                if ( openOther.value ) {
                    priceForm.kePress = (priceForm.inputPrice.start === '' || priceForm.inputPrice.end === '');
                    priceForm.price.start = parseFloat( priceForm.inputPrice.start ) > props.maxValue ? props.maxValue : parseFloat( priceForm.inputPrice.start ) || props.minValue;
                    priceForm.price.end = parseFloat( priceForm.inputPrice.end ) > props.maxValue ? props.maxValue : parseFloat( priceForm.inputPrice.end ) || props.maxValue;
                }
            }, 1),
            { deep: true }
        );
        ...
        const filterPriceLabel = ()=> {
            //console.log('boom', openOther.value);

            if(priceForm.price.start && priceForm.price.end) {
                //console.log('check when fire')
                //priceForm.price.start = priceForm.price.start || props.minValue;
                //priceForm.price.end = priceForm.price.end || props.maxValue;

                return `<span class="filter_dates">
                    ${priceForm.price.start} - ${priceForm.price.end} CHF
                    </span>
