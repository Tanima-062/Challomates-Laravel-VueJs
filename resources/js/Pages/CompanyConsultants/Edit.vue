<template>
    <div class="challo__card challo__card-p6 rounded-b-3x">
        <Back
            ...
            :staticBackdrop="true"
        />
        <h5 class="page-title">Company Consultant bearbeiten</h5>
...
import {maxLength, required, helpers, decimal, integer, alpha, email} from "@vuelidate/validators";
...
const props = defineProps(["company_consultant"]);
...
const form = useForm({
    consultantId: props.company_consultant.id,
    latest_id: props.company_consultant.prefix_id,
    creation_date: props.company_consultant.created_at,
    first_name: props.company_consultant.first_name,
    last_name: props.company_consultant.last_name,
    email_address: props.company_consultant.email,
    country_iso_code: props.company_consultant.country_iso_code,
    phone_number: props.company_consultant.phone_number,
    _method: "PUT",
});
...
const update = () => {
    if ( canSubmit ) {
        form.post(
            route("company-consultants.update", {
                company_consultant: props.company_consultant.id,
            }),
            {
                forceFormData: true,
